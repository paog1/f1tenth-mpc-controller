...
class RacingLineManager:
    ...
    def __init__(
        self,
        track: Track,
        frenet: FrenetProjector,
        ey_margin: float,
        log_dir: str,
        seed_laps: int,
        smooth_window: int,
        # MPC / Kinematic Limits
        vx_max: float,
        a_max: float,
        b_max: float,
        # Gradient Descent Parameters
        mu_max: float = 1.0,
        gd_learning_rate: float = 0.03,
        gd_eps: float = 0.01,
        gd_iterations: int = 10,
        gd_subsample: int = 4,
        # Safety QP (Optional post-processing)
        mincurv_enable: bool = False,
        mincurv_w_curv: float = 0.01,
        mincurv_w_smooth: float = 5.0,
        mincurv_w_data: float = 10.0,
        mincurv_w_prev: float = 0.0,
        **kwargs
    ):
        self.track = track
        self.frenet = frenet
        ...
        # Kinematics & Optimization Config
        self.vx_max = vx_max
        self.a_max = a_max
        self.b_max = b_max
        self.mu_max = mu_max
        ...
        self.gd_subsample = gd_subsample

        # Optional QP smoothing config
    ...
    def _gradient_descent_step(self, ey: np.ndarray, lr: float) -> np.ndarray:
        """Computes finite-difference gradient and updates the line."""
        N = len(ey)
        grad = np.zeros(N)
        eps = self.gd_eps
        
        # Subsample evaluations to maintain <0.1s compute time
        eval_indices = np.arange(0, N, self.gd_subsample)
        g_vals = np.zeros(len(eval_indices))
        
        # Calculate finite-difference gradient
        for idx_i, i in enumerate(eval_indices):
            # Apply a smooth bump to prevent physically impossible jagged kinks
            bump = np.zeros(N)
            bump[i] = eps
            bump = circ_moving_average(bump, 3) 
            bump *= (eps / max(bump)) 
            
            Tp = self._compute_lap_time(ey + bump)
            Tm = self._compute_lap_time(ey - bump)
            g_vals[idx_i] = (Tp - Tm) / (2 * eps)
            
        # Interpolate gradient safely across the periodic track
        x_eval = np.append(eval_indices, eval_indices[0] + N)
        y_eval = np.append(g_vals, g_vals[0])
        grad = np.interp(np.arange(N), x_eval, y_eval)
        
        # Smooth the gradient so the car updates the line in flowing curves
        grad = circ_moving_average(grad, self.smooth_window)
        
        # Update Rule
        ey_new = ey - lr * grad
        
        # Clamp to track boundaries and smooth
        ey_new = self._clip_to_walls(ey_new)
        ey_new = circ_moving_average(ey_new, self.smooth_window)
        
        return ey_new

    def _compute_lap_time(self, ey: np.ndarray) -> float:
        """
        Builds the global path from offsets, calculates its curvature mathematically, 
        runs a forward-backward kinematic pass, and returns the predicted lap time.
        """
        # 1. Map to Global Coordinates
        nx = -np.sin(self.track.theta)
        ny = np.cos(self.track.theta)
        X = self.track.x + nx * ey
        Y = self.track.y + ny * ey

        # 2. Compute Path Curvature (Vectorized exact formulation)
        X_pad = np.pad(X, 1, mode='wrap')
        Y_pad = np.pad(Y, 1, mode='wrap')
        
        dx = np.gradient(X_pad)[1:-1]
        dy = np.gradient(Y_pad)[1:-1]
        ddx = np.gradient(np.gradient(X_pad))[1:-1]
        ddy = np.gradient(np.gradient(Y_pad))[1:-1]
        
        # kappa = |x'y'' - y'x''| / (x'^2 + y'^2)^1.5
        ds_grad2 = dx**2 + dy**2
        kappa = np.abs(dx * ddy - dy * ddx) / np.maximum(ds_grad2**1.5, 1e-6)

        # 3. Calculate Distances between points
        ds_fw = np.sqrt((np.roll(X, -1) - X)**2 + (np.roll(Y, -1) - Y)**2)
        ds_bw = np.roll(ds_fw, 1)

        # 4. Friction Limit (Steady-State Cornering)
        v_max_fric = np.sqrt(self.mu_max * 9.81 / np.maximum(kappa, 1e-6))
        v = np.minimum(v_max_fric, self.vx_max)

        # 5. Forward-Backward Kinematic Pass (Accel / Decel Limits)
        a_max = self.a_max
        b_max = self.b_max  # Expected to be negative (e.g., -5.0)

        # Forward Pass (Run twice to propagate through the start/finish wrap-around)
        for _ in range(2):
            for i in range(1, len(v)):
                v[i] = min(v[i], math.sqrt(max(0.0, v[i-1]**2 + 2 * a_max * ds_bw[i])))
            v[0] = min(v[0], math.sqrt(max(0.0, v[-1]**2 + 2 * a_max * ds_bw[0])))

        # Backward Pass (Propagate braking zones backward)
        for _ in range(2):
            for i in range(len(v)-2, -1, -1):
                v[i] = min(v[i], math.sqrt(max(0.0, v[i+1]**2 - 2 * b_max * ds_fw[i])))
            v[-1] = min(v[-1], math.sqrt(max(0.0, v[0]**2 - 2 * b_max * ds_fw[-1])))

        # 6. Integrate Lap Time: sum(ds / v)
        T_pred = np.sum(ds_fw / np.maximum(v, 0.1))
        return float(T_pred)
    ...
    def prepare_next_candidate(self, stored_laps: int):
        """Generates the next lap's racing line strictly by minimizing Time."""
        ...
        T_start = self._compute_lap_time(ey_cand)
        
        for i in range(self.gd_iterations):
            ey_cand = self._gradient_descent_step(ey_cand, lr=self.gd_learning_rate)
            
        T_end = self._compute_lap_time(ey_cand)

        # Optional: Pass through the old QP purely to enforce strict kinematic smoothing.
        # This does NOT optimize for distance/curvature, it acts only as a low-pass spatial filter.
        if self.mincurv_enable:
            ey_cand = self._minimum_curvature_qp(ey_cand, base)

        self.candidate_line = ey_cand
        self.candidate_active = True
        self._update_dynamic_curvature() 
        
        self._save_line_csv(os.path.join(self.log_dir, f"candidate_line_lap_{stored_laps:03d}.csv"), ey_cand)
        print(f"[Time-Optimal GD] Lap {stored_laps} Candidate generated. Predicted Time: {T_start:.2f}s -> {T_end:.2f}s")
...
class IterativeRacingLineMPC(Node):
    ...
    def __init__(self):
        ...
        # Safety & Lap Tracking Boundaries
        self.declare_parameter('ey_margin', 0.2)
        ...
        # (Add these declarations near the top of your __init__ or params file)
        self.declare_parameter('mu_max', 1.0)
        self.declare_parameter('gd_learning_rate', 0.05)
        self.declare_parameter('gd_eps', 0.01)
        self.declare_parameter('gd_iterations', 10)
        self.declare_parameter('gd_subsample', 4)

        # Read them
        mu_max = float(self.get_parameter('mu_max').value)
        gd_lr = float(self.get_parameter('gd_learning_rate').value)
        gd_eps = float(self.get_parameter('gd_eps').value)
        gd_iters = int(self.get_parameter('gd_iterations').value)
        gd_sub = int(self.get_parameter('gd_subsample').value)
        ...
        self.Q = np.diag([
            float(self.get_parameter('q_vx').value), float(self.get_parameter('q_vy').value),
            float(self.get_parameter('q_wz').value), float(self.get_parameter('q_epsi').value),
            float(self.get_parameter('q_s').value), float(self.get_parameter('q_ey').value),
        ])
        self.Qf = np.diag([
            float(self.get_parameter('qf_vx').value), float(self.get_parameter('qf_vy').value),
            float(self.get_parameter('qf_wz').value), float(self.get_parameter('qf_epsi').value),
            float(self.get_parameter('qf_s').value), float(self.get_parameter('qf_ey').value),
        ])
        self.R = np.diag([float(self.get_parameter('r_delta').value), float(self.get_parameter('r_a').value)])
        ...
        self.get_logger().info(f"Track loaded: L={self.track.L:.2f} m | kappa[min,max]=({float(np.min(self.track.kappa)):.3f}, {float(np.max(self.track.kappa)):.3f})")
        ...
        self.line_mgr = RacingLineManager(
            ...
            vx_max=self.vx_max,
            a_max=self.a_max,
            b_max=self.a_min, # Assuming a_min is your negative braking limit (e.g., -5.0)
            mu_max=mu_max,
            # Pass GD params
            ...
            mincurv_w_curv=0.01, # Keep this VERY low so it doesn't fight the GD time-optimization
        ...
        self.vel_mgr = VelocityProfileManager(
            track=self.track, line_mgr=self.line_mgr, log_dir=self.log_dir, vx_min=self.vx_min, vx_max=self.vx_max,
            a_lat_ref_max=self.a_lat_ref_max, curv_speed_floor=self.curv_speed_floor, speed_ref_scale=self.speed_ref_scale,
            enable=self.velocity_learning_enable, blend=self.velocity_blend, smooth_window=self.velocity_smooth_window,
            accept_margin_sec=self.velocity_accept_margin_sec, data_margin=self.velocity_data_margin, max_speed_step=self.velocity_max_step,
        )
        ...
        with open(self.summary_csv, 'w', newline='') as f:
            wr = csv.writer(f)
            wr.writerow(['lap_idx', 'lap_time_sec', 'stored_in_memory', 'accepted_new_best', 'velocity_profile_updated', 'best_lap_time_sec', 'fail_ratio', 'near_wall_ratio', 'out_of_track_ratio'])
    ...
    def _load_track(self, path: str) -> Track:
        ...
        return Track(
            s=s.astype(float), kappa=kappa.astype(float), x=x.astype(float) if x is not None else None,
            y=y.astype(float) if y is not None else None, theta=theta.astype(float) if theta is not None else None,
            w_left=w_left.astype(float) if w_left is not None else None, w_right=w_right.astype(float) if w_right is not None else None
        )
    ...
    def solve_qp(self, P, q, A, l, u):
        prob = osqp.OSQP()
        prob.setup(
            P=P, q=q, A=A, l=l, u=u, verbose=False, warm_start=True,
            eps_abs=1e-3, eps_rel=1e-3, max_iter=20000, polish=True
        )
    ...
    def control_loop(self):
        ...
        if len(A_list) != self.N or len(B_list) != self.N or len(C_list) != self.N:
            self.get_logger().error("ATV length mismatch.")
            return
    ...
    def finalize_lap_if_needed(self, x0: np.ndarray):
        ...
        if steps < self.min_lap_steps:
            self.get_logger().warn(f"Discarding lap with only {steps} steps.")
            self.curr_traj_x = []
            self.curr_traj_u = []
            self.qp_fail_in_lap = 0
            self.near_wall_in_lap = 0
            ...
            if self.do_seed and self.current_lap_idx < self.seed_laps:
                self.get_logger().info(f"Next seed lap {self.current_lap_idx}: {self.seed_lap_label(self.current_lap_idx)}")
        ...
        if valid_lap:
            ...
            self.laps.append({
                'x': X, 'u': U, 'J': J, 'lap_time': lap_time,
                'fail_ratio': fail_ratio, 'near_wall_ratio': near_wall_ratio, 'out_ratio': out_ratio,
            })
            self.total_laps += 1
        ...
        if valid_lap and hasattr(self, 'vel_mgr') and self.current_lap_idx >= self.seed_laps:
            velocity_updated = self.vel_mgr.update_from_lap(
                lap_idx=self.current_lap_idx, lap_time=lap_time, valid_lap=valid_lap, X_lap=X,
            )

        accepted, best_after = self.line_mgr.finish_lap(
            lap_idx=self.current_lap_idx, stored_laps=len(self.laps), lap_time=lap_time, valid_lap=valid_lap, X_lap=X
        )

        with open(self.summary_csv, 'a', newline='') as f:
            wr = csv.writer(f)
            wr.writerow([
                self.current_lap_idx, lap_time, int(stored_in_memory), int(accepted),
                int(velocity_updated), best_after if np.isfinite(best_after) else -1.0,
                fail_ratio, near_wall_ratio, out_ratio
            ])
        ...
        if valid_lap:
            ...
            self.get_logger().warn(f"Lap {self.current_lap_idx} not stored | time={lap_time:.2f}s")
    ...
    def seed_lap_label(self, lap_idx: int) -> str:
        mode = lap_idx % 5
        if mode in (0, 1, 4): return "centerline"
        if mode == 2: return "left-biased"
        if mode == 3: return "right-biased"
        return "centerline"
