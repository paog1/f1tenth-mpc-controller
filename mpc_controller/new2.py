...
class RacingLineManager:
    ...
    def __init__(
        ...
        # --- NEW: SQP Pipeline States ---
        self.sqp_state = "IDLE" 
        self.sqp_candidate = None
        self.bg_thread = None
    ...
    def _minimum_curvature_qp(self, data_line: np.ndarray, prev_line: np.ndarray) -> np.ndarray:
        ...
        w_data_array = np.full(n, self.mincurv_w_data)
    ...
    # ===============================
    # Asynchronous SQP Pipeline
    # ===============================

    def start_sqp_optimization(self, stored_laps: int, X_lap: np.ndarray, current_velocity: np.ndarray):
        """Starts the offline SQP math in a background thread."""
        # Only start if we have finished seeding and the thread isn't already busy
        # fix the 0 
        if stored_laps < max(0, self.seed_laps):
            self.candidate_active = True
            self.candidate_line = self.best_line.copy()
            self._update_dynamic_curvature()
            return

        if hasattr(self, 'sqp_state') and self.sqp_state == "COMPUTING":
            print("[SQP] Warning: Previous computation still running. Skipping trigger.")
            return

        self.sqp_state = "COMPUTING"
        self.sqp_candidate = None
        
        # Launch the offline learning math
        self.bg_thread = threading.Thread(
            target=self._sqp_worker, 
            # --- THE FIX: Always pull the actively driven line, not the stale memory ---
            args=(stored_laps, X_lap, self.active_line().copy(), current_velocity), 
            daemon=True
        )
        self.bg_thread.start()
        print(f"[SQP] Started iterative minimum-time optimization for Lap {stored_laps} in background...")

    # def _sqp_worker(self, stored_laps: int, X_lap: np.ndarray, base_line: np.ndarray):
    #     """The core mathematical SQP loop. Runs entirely offline."""
    #     try:
    #         current_guess = base_line.copy()
    #         n_points = len(self.track.s)
            
    #         # SQP Loop Parameters
    #         max_sqp_iter = 5
    #         convergence_tol = 0.005 # meters
            
    #         decay_rate = 0.85
    #         laps_learning = stored_laps - self.seed_laps
            
    #         for sqp_step in range(max_sqp_iter):
    #             ''' MINIMUM TIME CODE (Using Dynamic Curvature as a Proxy)
    #             # # 1. Linearize the physics around the current guess
    #             # # To minimize time, we maximize allowable velocity: v <= sqrt(a_lat / kappa)
    #             # # We calculate the spatial derivative to find the non-linear gradient of curvature
                
    #             # # Smooth the guess to ensure stable derivatives
    #             # smooth_guess = circ_moving_average(current_guess, self.smooth_window)
                
    #             # # Approximate the gradient of track width usage (Path Length vs Curvature tradeoff)
    #             # # This is a mathematical proxy for the Jacobian of the time-objective
    #             # signed_dyn_kappa = self.dynamic_kappa * np.sign(self.track.kappa)
    #             # # --- REMOVED THE ** 3 CUBIC DAMPENER ---
    #             # apex_pull = self.exploration_amp * np.clip(signed_dyn_kappa / 0.6, -1.0, 1.0)                # 2. Formulate the temporary quadratic cost function (The Q and q matrices)
    #             # # Instead of completely replacing OSQP, we trick the existing QP into solving the linearized SQP step
    #             # # by injecting our calculated Jacobian as the 'data_target'.
    ...
    #             # # The gradient step (\Delta x)
    #             # delta_x = apex_pull + entry_push + exit_push
    #             # target_step = smooth_guess + delta_x
                
    #             # # 3. Solve the Quadratic sub-problem subject to kinematic bounds
    #             # next_guess = self._minimum_curvature_qp(target_step, smooth_guess)
                
    #             # # 4. Check for mathematical convergence
    #             # max_change = np.max(np.abs(next_guess - current_guess))
    #             # current_guess = next_guess.copy()
                
    #             # if max_change < convergence_tol:
    #             #     print(f"[SQP] Mathematically converged after {sqp_step + 1} iterations. Max shift: {max_change:.4f}m")
    #             #     break
                    
    #             # # Yield thread briefly so the live MPC node doesn't stutter
    #             # time.sleep(0.01)'''



    #             ''' MINIMUM CURVATURE CODE
    #             smooth_guess = circ_moving_average(current_guess, self.smooth_window)

    #             macro_dyn_kappa = circ_moving_average(self.dynamic_kappa, int(self.smooth_window * 3))
    #             signed_dyn_kappa = macro_dyn_kappa * np.sign(self.track.kappa)
                
    #             # --- 2. PURE APEX PULL ---
    #             # Push the line strictly to the INSIDE of the curve to flatten it.
    #             apex_pull = self.exploration_amp * np.clip(signed_dyn_kappa / 0.6, -1.0, 1.0)
                
    #             # --- 3. LATE APEX PHASE SHIFT (The V-Line Creator) ---
    #             # Shift the mathematical pull roughly 2.5 meters downstream.
    #             # This delays the turn-in, creating an asymmetric late apex for faster exits.
    #             ds_nom = float(np.median(np.diff(self.track.s))) if len(self.track.s) > 2 else 0.1
    #             late_apex_shift = int(2.5 / ds_nom) 
                
    #             # We completely abandon the hardcoded entry_push and exit_push to 
    #             # prevent destructive interference. Let the QP rubber-band the rest!
    #             delta_x = np.roll(apex_pull, late_apex_shift)
                
    #             target_step = smooth_guess + delta_x
                
    #             # Solve the Quadratic sub-problem subject to kinematic bounds
    #             next_guess = self._minimum_curvature_qp(target_step, smooth_guess)
    ...
    #         # Optimization complete. Store result safely.
    #         self.sqp_candidate = current_guess
    #         self.sqp_state = "READY"
    #         print(f"[SQP] Background computation complete. Ready for Hot-Swap.")
            
    #     except Exception as e:
    #         print(f"[SQP] Mathematical divergence in offline solver: {e}")
    #         self.sqp_candidate = base_line.copy() # Fallback to safe line
    #         self.sqp_state = "READY"

    def _sqp_worker(self, stored_laps: int, X_lap: np.ndarray, base_line: np.ndarray, current_velocity: np.ndarray):
        """The core mathematical mutation loop. Runs offline EXACTLY ONCE per lap."""
        try:
            # --- 1. GEOMETRY (The Distance Component) ---
            smooth_kappa = circ_moving_average(self.track.kappa, self.smooth_window * 2)
            max_k = np.max(np.abs(smooth_kappa)) + 1e-6
            norm_k = np.abs(smooth_kappa) / max_k
            
            # The base pull always wants to hug the inside of the corner
            apex_pull = np.sign(smooth_kappa) * (norm_k ** 2)
            
            # --- 2. KINEMATICS (The Speed Component) ---
            # Calculate the spatial derivative of velocity (dv/ds)
            dv_ds = np.gradient(current_velocity, self.track.s)
            norm_dv = np.abs(dv_ds) / (np.max(np.abs(dv_ds)) + 1e-6)
            
            # Whenever the car is braking or accelerating heavily, push it to the OUTSIDE.
            # Outside is the exact mathematical opposite of the apex_pull.
            velocity_push = -np.sign(smooth_kappa) * norm_dv
            
            # --- 3. THE TIME-OPTIMAL GRADIENT ---
            # Weight the velocity push slightly higher (1.5x) so exit speed dominates distance.
            time_optimal_gradient = apex_pull + (1.5 * velocity_push)
            
            # Decay the amplitude over time as it approaches the physical walls
            decay_rate = 0.90
            laps_learning = stored_laps - self.seed_laps
            current_amp = self.exploration_amp * (decay_rate ** laps_learning)
            current_amp = max(current_amp, 0.20) 
            
            # Apply the gradient
            delta_x = current_amp * time_optimal_gradient
            target_step = base_line + delta_x
            
            # The QP ensures kinematic smoothness and clips it safely inside the track walls
            next_guess = self._minimum_curvature_qp(target_step, base_line)
            
            self.sqp_candidate = next_guess.copy()
            self.sqp_state = "READY"
            print(f"[SQP] Time-Optimal Gradient computed with amplitude {current_amp:.2f}m.")
            
        except Exception as e:
            print(f"[SQP] Mathematical divergence in offline solver: {e}")
            self.sqp_candidate = base_line.copy() 
            self.sqp_state = "READY"

    def execute_hot_swap(self) -> bool:
        """Called by the main node exactly when crossing the start/finish line."""
        if hasattr(self, 'sqp_state') and self.sqp_state == "READY" and self.sqp_candidate is not None:
            
            self.best_line = self.sqp_candidate.copy()
            self.candidate_line = self.sqp_candidate.copy()
            self.candidate_active = True
            
            self._update_dynamic_curvature()
            self.sqp_state = "IDLE"
            print("[SQP] >>> HOT-SWAP EXECUTED: New optimal trajectory actively tracking! <<<")
            return True
        return False
    ...
    def finish_lap(
        ...
        if valid_lap:
            good_enough = (
                not np.isfinite(self.best_lap_time)
                or lap_time <= self.best_lap_time + self.accept_margin_sec
            )
            ...
            if good_enough:
                ...
                if not np.isfinite(self.best_lap_time) or lap_time < self.best_lap_time:
                    self.best_lap_time = lap_time
                    self.best_lap_idx = lap_idx
                    print(f"[PROGRESS] New absolute speed record: {lap_time:.2f}s.")
                else:
                    print(f"[PROGRESS] Geometry accepted within margin ({lap_time:.2f}s).")

            else:
                print(
                    f"[PROGRESS] Valid but too slow: {lap_time:.2f}s "
                    f"> best {self.best_lap_time:.2f}s + margin {self.accept_margin_sec:.2f}s. Reverting."
                )
                self.candidate_line = self.best_line.copy()
                self.candidate_active = False
                accepted = False
            ...
            print("[PROGRESS] Lap failed. Reverting to last safe geometry.")
            self.candidate_line = self.best_line.copy()
            self.candidate_active = False
            accepted = False
...
class IterativeRacingLineMPC(Node):
    def __init__(self):
        ...
        self.declare_parameter('a_bounds', [-6.0, 3.5])
        self.declare_parameter('delta_rate_max', 2.0)
        self.declare_parameter('a_rate_max', 5.0)
        self.declare_parameter('rho_du', 0.2)

        self.declare_parameter('wheelbase', 0.33)
        self.declare_parameter('steer_sign', 1.0)
        self.declare_parameter('ff_gain', 1.0)

        self.declare_parameter('do_seed_laps', False)
        ...
        self.declare_parameter('exploration_amp', 1.5)  # Reduced from 3.0 to 2.0 to prevent over-aggressive shifts
        self.declare_parameter('exploration_sectors', 8)
        self.declare_parameter('sectors_to_perturb', 10)  
        self.declare_parameter('accept_margin_sec', 0.2)
        ...
        self.declare_parameter('mincurv_w_data', 5.0)
        ...
        self.declare_parameter('velocity_learning_enable', False)
        self.declare_parameter('velocity_blend', 0.35)
        self.declare_parameter('velocity_smooth_window', 21)
        self.declare_parameter('velocity_accept_margin_sec', 0.2)
    ...
    def atv_matrices(self, xbar_seq: np.ndarray, ubar_seq: np.ndarray):
        """
        Linearized discrete Frenet kinematic bicycle model.
        ...
        Important modelling choice:
            wz is still kept as a state for diagnostics/costs,
            but epsi dynamics use the algebraic bicycle yaw rate:
                wz_alg = vx * tan(delta) / L
            not the delayed predicted wz state.
        """
        ...
        for k in range(self.N):
            ...
            # Jacobian of s_dot w.r.t. states
            dfs = np.zeros(6)
            dfs[0] = cos_e / den
            dfs[1] = -sin_e / den
            ...
            # Jacobian of ey_dot w.r.t. states
            dfey = np.zeros(6)
            dfey[0] = sin_e
            dfey[1] = cos_e
            dfey[3] = vx * cos_e - vy * sin_e
            ...
            # vy is forced to zero in this kinematic model
            A[1, :] = 0.0
            B[1, :] = 0.0
            C[1] = 0.0

            # wz_{k+1} = vx_k * tan(delta_k) / L
            A[2, 0] = tan_del / self.L_wb
            B[2, 0] = vx * sec2_del / self.L_wb
            C[2] = wz_alg - A[2, 0] * vx - B[2, 0] * delta_ref

            # epsi_{k+1} = epsi_k + dt * epsi_dot
    ...
    def build_reference_horizon(self, x0: np.ndarray):
        ...
        for k in range(self.N + 1):
            ...
            kappa_c = self.track.kappa_at(s_k)
            denom = max(0.1, 1.0 - kappa_c * ey_k)
            epsi_ref = math.atan2(deyds, denom)
    ...
    def finalize_lap_if_needed(self, x0: np.ndarray):
        ...
        if steps < self.min_lap_steps:
            ...
            # self.line_mgr.start_sqp_optimization(len(self.laps), X)
        ...
        if valid_lap and hasattr(self, 'vel_mgr') and self.current_lap_idx >= self.seed_laps:
            ...
            if near_wall_ratio < 0.10 and out_ratio == 0.0 and fail_ratio == 0.0:
                self.vel_mgr.a_lat_ref_max += 0.02
                self.get_logger().info(f"[LIMIT PUSH] Track feels safe. Pushing grip limit up to {self.vel_mgr.a_lat_ref_max:.2f}")
                
            # We ONLY drop the limit if the car is literally failing to stay on the track 
            # or if the QP solver is failing to find feasible solutions.
            elif out_ratio > 0.0 or fail_ratio > 0.0:
                self.vel_mgr.a_lat_ref_max = max(1.0, self.vel_mgr.a_lat_ref_max - 0.05)
                self.get_logger().warn(f"[LIMIT PUSH] Physical limit exceeded! Dropping grip back to {self.vel_mgr.a_lat_ref_max:.2f}")
        ...
        self.line_mgr.start_sqp_optimization(len(self.laps), X, self.vel_mgr.learned_speed.copy())

        # Update the RViz display with the actively executing line
        self.publish_learned_line()
    ...
    def control_loop(self):
        ...
        # fix the value down here
        if float(x0[4]) < 16.0:
            if hasattr(self.line_mgr, 'sqp_state') and self.line_mgr.sqp_state == "READY":
                self.line_mgr.execute_hot_swap()
